...
# First, by HW1: E(X) = (1-p)/p = 1/p - 1 <= do we need this step? Not at all!
set.seed(11)
X <- rgeom(10000, 1/(pi+1))
getwd()
wd_path <- "/Users/Andy 1/Google 雲端硬碟 (r08323004@g.ntu.edu.tw)/0 Semesters/109-1/1 三345 統計TA/3 實習課教材/3 R/L4/data"
setwd(wd_path)
write.csv(X, "practice1.csv")
read.csv("practice1.csv")
mean(X)
...
# method 1
mll <- function(nu_1, nu_2){
  logLikelihood <-0
  for(x in X){
    logLikelihood <- logLikelihood + log(df(x, df1 = nu_1, df2 = nu_2))
  }
  return(-logLikelihood)
}
mll(2020,1223)
# the numerical maximization highly depends on the starting points
# try randomize the initial points
MaxLikeEst = mle(mll, start = list(nu_1 = round(runif(1, 1, 10000)), nu_2 = round(runif(1, 1, 10000))))
summary(MaxLikeEst)

MaxLikeEst = mle(mll, start = list(nu_1 = 2000, nu_2 = 1000))
summary(MaxLikeEst)
...
var(X)
# 95% C.I.
(length(X)-1)*var(X)/qchisq(p = 0.05/2, df = length(X)-1, lower.tail = F)
(length(X)-1)*var(X)/qchisq(p = 1-0.05/2, df = length(X)-1, lower.tail = F)

confidence_interval = function(n=100, alpha=0.05){
  X <- rnorm(n, mean = pi, sd = sqrt(pi))
  l = (length(X)-1)*var(X)/qchisq(p = 0.05/2, df = length(X)-1, lower.tail = F)
  u = (length(X)-1)*var(X)/qchisq(p = 1-0.05/2, df = length(X)-1, lower.tail = F)
  return(c(l, u))
}
res <- replicate(n = 100, expr = confidence_interval(), simplify = T)
res <- as.data.frame(t(res))
res <- res %>% 
  select(L = V1,
         U = V2) %>% 
  mutate(lie_in_interval = ifelse(L<=pi & U>=pi, 1, 0))
res %>% View
sum(res$lie_in_interval)/100

# Plot
ggplot(res, aes(x = 1:100, y = pi)) +
  geom_point(size = 1) +
  geom_errorbar(aes(ymax = U, ymin = L))
##### Practice 3 Ends #####
...
abline(v = mean(X), col = 'royalblue') #the random sample we saw, which is varphi_0
#?polygon
...
X <- rnorm(100, pi, sqrt(pi))
# Let's assume mu & sigma^2 are unknown

# H0: sigma^2 = 3
# Ha: sigma^2 != 3
var(X)
# Test statistics, i.e. varphi_0 (textbook p.271)
(length(X)-1)*var(X)/3 > qchisq(p = 0.05/2, df = length(X)-1, lower.tail = F)
(length(X)-1)*var(X)/3 < qchisq(p = 1-0.05/2, df = length(X)-1, lower.tail = F)

grid <- seq(0, 200, length = 1000)
plot(grid, dchisq(x = grid, 
                 df = length(X)-1), #under H0
     type = "l",
     xlab = "varphi",
     ylab = "")
#under the null hypothesis (textbook p.271)
abline(v = (length(X)-1)*var(X)/3, col = 'royalblue') #the random sample we saw, which is varphi_0
#?polygon
polygon(c((length(X)-1)*var(X)/3, grid[grid>=(length(X)-1)*var(X)/3]),
        c(0, dchisq(grid[grid>=(length(X)-1)*var(X)/3], df=length(X)-1,)),
        col = "royalblue")
abline(v = qchisq(0.05/2, df = length(X)-1,lower.tail = F), col = 'red') #given the significance level: alpha = 0.05
abline(v = qchisq(1-0.05/2, df = length(X)-1,lower.tail = F), col = 'red')#RR in the left part
print(var(X))
